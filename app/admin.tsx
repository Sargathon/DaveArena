...
export default function AdminScreen() {
  ...
  if (!user?.isAdmin) {
    return (
      <SafeAreaView edges={['top']} style={[styles.container, { justifyContent: 'center', alignItems: 'center' }]}>
        ...
        <Text style={{ color: theme.textSecondary, marginTop: 12, fontSize: 16 }}>Accès admin requis</Text>
  ...
  const handleApprove = (id: string) => {
    showAlert('Confirmer', 'Approuver ce paiement et activer l\'accès ?', [
  ...
  const handleManualActivate = (userId: string, currentTier: UserTier) => {
    showAlert('Activer un plan', `Choisir le plan pour ${userId}`, [
      ...TIER_OPTIONS.map(opt => ({
        ...
        onPress: () => {
          ...
          Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
          showAlert('Activé', `${userId} est maintenant ${opt.label}`);
        },
      })),
            ...
            {registeredUsers.map((u, index) => (
              <Animated.View key={u.bookmakerId} entering={FadeInDown.delay(index * 50).duration(300)}>
                <Pressable
                  ...
                  onPress={() => handleManualActivate(u.bookmakerId, u.tier)}
                >
                  ...
                  <View style={[
                    ...
                    <Text style={[
                      ...
                      {u.tier === 'free' ? 'Free' : u.tier === 'basic' ? 'Basic' : u.tier === 'premium' ? 'Premium' : u.tier === 'vip' ? 'VIP' : 'Admin'}
                    </Text>
                  </View>
...
const styles = StyleSheet.create({
  ...
  approveBtn: {
    ...
    backgroundColor: theme.success, borderRadius: theme.radius.sm, paddingVertical: 10,
  },
  approveBtnText: { fontSize: 13, fontWeight: '700', color: '#FFF' },
  rejectBtn: {
    flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6,
    backgroundColor: 'rgba(239,68,68,0.1)', borderRadius: theme.radius.sm, paddingVertical: 10,
    borderWidth: 1, borderColor: 'rgba(239,68,68,0.3)',
  },
  rejectBtnText: { fontSize: 13, fontWeight: '600', color: theme.error },
  userCard: {
    flexDirection: 'row', alignItems: 'center', backgroundColor: theme.surface,
    borderRadius: theme.radius.md, padding: 14, marginBottom: 8,
    borderWidth: 1, borderColor: theme.border,
  },
  userAvatar: {
